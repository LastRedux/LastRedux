...
LRTrack LRAPIWrapper::parseTrack(const QJsonObject& trackObj) const {
	...
	QJsonArray image = albumObj["image"].toArray();

	// "image" from the last.fm API is actually an array
	// of image sizes, we only care about the largest one
	if(!image.isEmpty()){
		info.image = image.last().toObject()["#text"].toString();
	}
