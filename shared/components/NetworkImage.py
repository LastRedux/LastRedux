...
class NetworkImage(QtQuick.QQuickItem):
    ...
    def updatePaintNode(self, old_node, data):
        if self.__has_image:
            ...
            if (
                ...
                texture_node.setSourceRect(
                    slice_x, 0, slice_width, texture_size.height()
                )
                ...
                texture_node.setSourceRect(
                    0, slice_y, texture_size.width(), slice_height
                )
    ...
    hasImage = QtCore.Property(
        bool, lambda self: self.__has_image, notify=has_image_changed
    )
    ...
    source = QtCore.Property(
        QUrl, lambda self: self.__source, set_source, notify=source_changed
    )
