...
class FriendsListModel(QtCore.QAbstractListModel):
    ...
    def set_friends_reference(self, newReference: FriendsViewModel) -> None:
        if newReference:
            ...
            self.__friends_reference.begin_refresh_friends.connect(
                lambda: self.beginResetModel()
            )
            self.__friends_reference.end_refresh_friends.connect(
                lambda: self.endResetModel()
            )
            self.__friends_reference.album_image_url_changed.connect(
                self.__track_album_image_url_changed
            )
    ...
    def rowCount(
        ...
        int
    ):  # parent would default to None but C++ requires us to use an empty QModelIndex instead
    ...
    def data(
        self, index: int, role: int = QtCore.Qt.DisplayRole
    ) -> Union[
        str, bool
        ...
        if self.__friends_reference:
            ...
            if index.isValid():
                ...
                friend = self.__friends_reference.friends[
                    index.row()
                ...
                if role == self.__USERNAME_ROLE:
                    ...
                    return (
                        friend.last_scrobble.track_title if friend.last_scrobble else ""
                    )
                elif role == self.__TRACK_ARTIST_NAME_ROLE:
                    return (
                        friend.last_scrobble.artist_name if friend.last_scrobble else ""
                    )
                elif role == self.__TRACK_ARTIST_URL_ROLE:
                    return (
                        friend.last_scrobble.artist_url if friend.last_scrobble else ""
                    )
                elif role == self.__TRACK_IMAGE_URL:
                    return (
                        friend.last_scrobble.image_url if friend.last_scrobble else ""
                    )
                elif role == self.__IS_TRACK_PLAYING_ROLE:
                    return (
                        friend.last_scrobble.is_playing
                        if friend.last_scrobble
                        else False
                    )
                elif role == self.__IS_TRACK_LOVED_ROLE:
                    return (
                        friend.last_scrobble.is_loved if friend.last_scrobble else False
                    )
    ...
    friendsReference = QtCore.Property(
        type=FriendsViewModel, fget=get_friends_reference, fset=set_friends_reference
    )
