...
class LastfmApiWrapper:
    ...
    def get_user_info(self) -> LastfmUserInfo:
        return self.__lastfm_request(
            {"method": "user.getInfo", "username": self.username},
    ...
    def get_recent_scrobbles(
        self, limit: int, from_date: datetime = None, username: str = None
        ...
        args = {
            ...
            "username": username
            or self.username,  # Default arg value can't refer to self
            "limit": limit,
        }
        ...
        if from_date:
            args["from"] = int(
                from_date.timestamp()
            )  # Convert to int to trim decimal points (Last.fm doesn't like them)
    ...
    def get_total_loved_tracks(self) -> int:
        return self.__lastfm_request(
            ...
            return_value_builder=lambda response: int(
                response["lovedtracks"]["@attr"]["total"]
            ),
        )
    ...
    def get_friends(self) -> List[LastfmUser]:
        ...
        try:
            friends = self.__lastfm_request(
                {"method": "user.getFriends", "username": self.username},
                ...
                return_value_builder=lambda friends, response: [
                    LastfmUser(
                        ...
                        image_url=friend["image"][0][
                            "#text"
                        ],  # All sizes are the same apparently
                    )
    ...
    def get_top_artists(
        self, limit: int, period: str = "overall"
    ) -> LastfmList[LastfmArtist]:
        return self.__lastfm_request(
            {
            ...
            return_value_builder=lambda tracks, response: [
                LastfmTrack(
                    ...
                    artist_link=LastfmArtistLink(
                        url=track["artist"]["url"], name=track["artist"]["name"]
                    ),
            ...
            return_value_builder=lambda albums, response: [
                LastfmAlbum(
                    ...
                    artist_link=LastfmArtist(
                        url=album["artist"]["url"], name=album["artist"]["name"]
                    ),
    ...
    def get_artist_info(self, artist_name: str, username: str = None) -> LastfmArtist:
        return self.__lastfm_request(
            ...
            return_value_builder=lambda artist, response: LastfmArtist(
                ...
                .split(" <")[0]
                .strip(),  # Remove the "Read more on Last.fm" html link at the end
                tags=[self.__tag_to_lastfm_tag(tag) for tag in artist["tags"]["tag"]],
                similar_artists=[
                    LastfmArtist(name=similar_artist["name"], url=similar_artist["url"])
    ...
    def get_track_info(
        self, artist_name: str, track_title: str, username: str = None
    ) -> LastfmTrack:
        return self.__lastfm_request(
            {
            ...
            return_value_builder=lambda track, response: LastfmTrack(
                ...
                title=track["name"],
                artist_link=LastfmArtistLink(
                    name=track["artist"]["name"], url=track["artist"]["url"]
                ),
                ...
                tags=[
                    LastfmApiWrapper.__tag_to_lastfm_tag(tag)
    ...
    def get_album_info(
        self, artist_name: str, album_title: str, username: str = None
    ) -> LastfmAlbum:
        if not album_title:
            return
        ...
        return self.__lastfm_request(
            ...
            return_value_builder=lambda album, response: LastfmAlbum(
                url=album["url"],
                title=album["name"],
                artist_link=LastfmArtist(url=None, name=album["artist"]),
                image_set=LastfmApiWrapper.__images_to_image_set(album["image"]),
    ...
    def log_in_with_session(self, session: LastfmSession) -> None:
        set_user({"username": session.username})

        # TODO: Follow guidelines for OS context
        set_context("system_profile", {**helpers.generate_system_profile()})
        set_context("app", {"app_version": "Beta 3"})
    ...
    def set_track_is_loved(
        self, artist_name: str, track_title: str, is_loved: bool
    ) -> LastfmSubmissionStatus:
        return self.__lastfm_request(
            {
    ...
    def get_friend_scrobble(self, username: str) -> FriendScrobble:
        def __track_to_friend_track(track):
            ...
            )  # 'true' when true, misssing when false
        ...
        try:
            friend_scrobble = self.__lastfm_request(
                ...
                main_key_getter=lambda response: (
                    response["recenttracks"]["track"][0]
                    if len(response["recenttracks"]["track"])
                    else None
                ),  # Not all users have a scrobble
                return_value_builder=lambda track, response: (
                    __track_to_friend_track(track) if track else None
                ),
            )
    ...
    def __lastfm_request(
        ...
        params = {"api_key": LastfmApiWrapper.API_KEY, "format": "json", **args}
        ...
        for _ in range(LastfmApiWrapper.MAX_RETRIES):
            ...
            try:
                if main_key_getter:
                    return_object = return_value_builder(
                        main_key_getter(resp_json), resp_json
                    )
                else:
                ...
                logging.debug(
                    f"Mising key in Last.fm request: {str(err)} for request {args}"
                )
                continue
            ...
            raise Exception(
                f'Could not request {args["method"]} after {LastfmApiWrapper.MAX_RETRIES} retries'
            )
    ...
    def __tag_to_lastfm_tag(tag: dict) -> LastfmTag:
        return LastfmTag(name=tag["name"], url=tag["url"])

    @staticmethod
    def __images_to_image_set(images: List[dict]) -> ImageSet:
        return ImageSet(
            small_url=images[1]["#text"] or None, medium_url=images[-1]["#text"] or None
        )
