...
class SpotifyApiWrapper:
    ...
    def get_track_images(
        ...
        results = self.__search(query=query, media_type="track")
        ...
        if results:
            ...
            if track:
                spotify_data.album_art = ImageSet(
                    ...
                    medium_url=track["album"]["images"][-2][
                        "url"
            ...
            if not is_retry and album_title:
                return self.get_track_images(
                    artist_name, track_title, None, only_album_art, is_retry=True
                )  # Retry 0 because the parameters have changed
            else:
    ...
    def __search(self, query: str, media_type: str, limit: int = 20) -> ImageSet:
        result = self.__request(
            ...
            args={"q": query, "type": media_type, "limit": limit},
        )
    ...
    def __request(self, url: str, args: dict, is_retry=False) -> dict:
        ...
        self.__ram_cache[request_string] = CachedResource(
            data=resp_json, expiration_date=None  # Spotify data shouldn't change
        )
    ...
    def __find_track_match(results: List[dict], artist_string: str) -> dict:
        ...
        for match in results:
            for artist in match["artists"]:
                ...
                nuked_artist_string = SpotifyApiWrapper.__nuke_artist_name(
                    artist_string
                )
                nuked_spotify_artist = SpotifyApiWrapper.__nuke_artist_name(
                    artist["name"]
                )
    ...
    def __get_access_token() -> dict:
        ...
        return requests.post(
            ...
            auth=(SpotifyApiWrapper.CLIENT_ID, SpotifyApiWrapper.CLIENT_SECRET),
        ).json()["access_token"]
    ...
    def __simplify_title(title: str) -> str:
        ...
        title = (
            title.replace(" - single", "").replace(" - ep", "").replace("edition", "")
        )
