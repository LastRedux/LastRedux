...
class HistoryViewModel(QtCore.QObject):
    ...
    __INITIAL_SCROBBLE_HISTORY_COUNT = int(
        os.environ.get("INITIAL_HISTORY_ITEMS", 30)
    )  # 30 is the default but can be configured
    ...
    def __init__(self) -> None:
        ...
        self.__is_spotify_plugin_available = (
            False  # This is set to true if Spotify is installed
        )
            ...
            if not os.environ.get("MOCK"):
                if media_player_preference == "spotify":
                    ...
                    if self.__is_spotify_plugin_available:
                        self.switchToMediaPlugin(
                            "spotify", should_update_in_database=False
                        )
                    else:
                        logging.info(
                ...
                if media_player_preference == "musicApp":
                    self.switchToMediaPlugin(
                        "musicApp", should_update_in_database=False
                    )
    ...
    @QtCore.Slot(str)
    def switchToMediaPlugin(
        self, media_plugin_name: str, should_update_in_database: bool = True
    ) -> None:
    ...
    def __connect_media_player_signals(self):
        ...
        self.__media_player.showNotification.connect(
            lambda title, content: self.__application_reference.showNotification.emit(
                title, content
            )
        )
    ...
    def __run_discord_rpc_request(self, request):
        ...
        if self.__connect_discord_rpc_if_open():
            ...
            while (
                not did_request_succeed
                and number_of_retries < self.__DISCORD_RPC_MAX_RETRIES
            ):
                try:
                    request(self)
    ...
    def __update_discord_rpc(
        ...
        self.__discord_rpc.update(
            ...
            start=(
                datetime.now() - timedelta(seconds=player_position)
            ).timestamp(),  # Don't include track start to accurately reflect timestamp in uncropped track
        )
    ...
    def __submit_scrobble(self, scrobble: Scrobble) -> None:
        ...
        if not self.__selected_scrobble_index is None:
            ...
            if (
                self.__selected_scrobble_index
                > HistoryViewModel.__CURRENT_SCROBBLE_INDEX
            ):
    ...
    def __handle_player_position_fetched(self, player_position: float) -> None:
        ...
        if player_position == self.__cached_playback_position:
            ...
            if (
                self.__ticks_since_position_change == 60
                and self.__is_discord_rpc_enabled
            ):
        ...
        self.__current_scrobble_percentage = (
            self.__determine_current_scrobble_percentage()
        )
        self.scrobble_percentage_changed.emit()
    ...
    def __determine_current_scrobble_percentage(self) -> int:
        ...
        relative_position = (
            self.__furthest_player_position_reached - self.__current_track_crop.start
        )
        relative_track_length = (
            self.__current_track_crop.finish - self.__current_track_crop.start
        )
        min_scrobble_length = (
            relative_track_length * 0.75
        )  # TODO: Grab the percentage from settings
    ...
    def __handle_media_player_playing(
        self, new_media_player_state: MediaPlayerState
    ) -> None:
    ...
    currentScrobble = QtCore.Property(
        type="QVariant",
        fget=lambda self: (
            asdict(self.__current_scrobble) if self.__current_scrobble else None
        ),
