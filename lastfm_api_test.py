...
if session:
    ...
    logging.info(
        f"Logged in from database as {session.username} with {session.session_key}"
    )
else:
    print("\n***** AUTH TOKEN *****\n")
    ...
    try:
        ...
        lastfm.log_in_with_session(session)
        logging.info(
            f"Successfully logged in as {session.username} with {session.session_key}"
        )
...
if (
    input(
        "Run POST requests? (yN) [WARNING: THESE WILL SUBMIT THINGS TO YOUR LAST.FM ACCOUNT] "
    )
    == "y"
):
    print("\n***** SUBMIT SCROBBLE *****\n")
    print(
        lastfm.submit_scrobble(
    ...
    print(
        lastfm.update_now_playing(
            artist_name="LASTREDUX TEST ARTIST", track_title="LASTREDUX TEST SONG 0"
        )
    )
