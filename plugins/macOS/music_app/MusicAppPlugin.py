...
class MusicAppPlugin(MacMediaPlayerPlugin):
    ...
    def __init__(self) -> None:
        ...
        self.__default_center.addObserver_selector_name_object_(
            self, "__handleNotificationFromMusic:", "com.apple.Music.playerInfo", None
        )
    ...
    def request_initial_state(self) -> None:
        ...
        is_playing = (
            self.__applescript_app.playerState() == MusicAppPlugin.PLAYING_STATE
        )
    ...
    def __handle_new_state(
        ...
        if not new_state.track_title:
            self.showNotification.emit(
                "Track cannot be scrobbled",
                f"Music did not provide any data for the media you're playing",
            )
            return

        # Apple Music puts Connecting... state string in the track title field for some reason
        if (
            new_state.track_title == "Connecting…"
        ):  # TODO: Find a better way to check that doesn't break other languages, songs named Connecting...
            self.stopped.emit()
            return
        ...
        if not new_state.artist_name:
            ...
            self.showNotification.emit(
                "Track cannot be scrobbled",
                f'Music did not provide an artist name for "{new_state.track_title}"',
            )
            return
        ...
        if is_library_track:
            ...
            if self.__last_notification_had_error:
                if (
                    ...
                    and self.__last_state_with_error.track_title
                    == new_state.track_title
                ):
                    self.showNotification.emit(
    ...
    def __launch_fetch_track_crop_task(self) -> None:
        ...
        get_library_track_crop.finished.connect(
            self.__handle_completion_of_get_track_crop_request
        )
        QtCore.QThreadPool.globalInstance().start(get_library_track_crop)

    def __handle_completion_of_get_track_crop_request(
        self, track_crop: TrackCrop
    ) -> None:
        ...
        if track_crop.finish != 0:
            ...
            if (track_crop.finish - track_crop.start) > 30.0:
                ...
                self.showNotification.emit(
                    "Track cannot be scobbled", "Track length is less than 30 seconds"
                )
        else:
    ...
    def __handleNotificationFromMusic_(
        self, notification
    ) -> None:  # TODO: Add type annotation
