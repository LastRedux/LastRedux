...
class MockPlayerPlugin(QtCore.QObject):
    ...
    MOCK_TRACKS = (
        json.load(open("mock_data/mock_tracks.json"))
        + [
            {
            ...
            {"reason": "Test song with no artist", "track_title": "localtrack.mp3"},
        ]
    ...
    def __get_player_state(self) -> MediaPlayerState:
        mock_track = MockPlayerPlugin.MOCK_TRACKS[
            self.__track_index % len(MockPlayerPlugin.MOCK_TRACKS)
        ]
        ...
        return MediaPlayerState(
            ...
            track_crop=TrackCrop(start=0, finish=MockPlayerPlugin.MOCK_TRACK_LENGTH),
        )
